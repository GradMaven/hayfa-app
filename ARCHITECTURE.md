...
- **Malware scanning is the one provider that defaults to real, not mock** — a health platform's document upload is exactly the wrong place for "scanning is off unless you opt in." `MALWARE_SCAN_PROVIDER=clamav` (real ClamAV, fails uploads closed if unreachable) is the default; a mock/skip is the explicit opt-out. The clamd wire protocol is implemented directly rather than through the `clamdjs` package this started with, after live testing against a real daemon showed that package silently mis-parsing replies — see [`docs/security-architecture.md`](docs/security-architecture.md) "Malware scanning on upload".
