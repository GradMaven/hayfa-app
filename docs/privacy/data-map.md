...
- **SMS vendor (Africa's Talking)** — once `SMS_PROVIDER=africastalking` is configured with real credentials; the local/default `console` mode sends nothing anywhere. What transits the vendor: the recipient's phone number and a short notification pointer, capped to 160 characters (never full clinical detail — see §39 and `src/lib/sms/index.ts`'s `buildSmsBody()`). Phone numbers are collected at sign-up (optional) or later via `/settings`; see [notification-architecture.md](../notification-architecture.md).
