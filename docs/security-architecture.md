...
## Data correction workflow (§56)

A direct `PATCH` on a `Medication`, `Condition`, or `LabResult` is rejected with `409 CONFLICT` once the record is no longer purely patient-asserted — `source !== "PATIENT_ENTERED"` (a provider, integration, or OCR originated it) or `verificationStatus === "PROVIDER_VERIFIED"` (`assertDirectEditAllowed()` in [`src/lib/api/patient-scope.ts`](../src/lib/api/patient-scope.ts), called from each resource's `PATCH` handler before the update). The patient's own freshly-entered, unverified data can still be edited in place — there's no provenance dispute to preserve there.

Disputing one of those locked records goes through `POST /api/v1/corrections` ([`src/app/api/v1/corrections/route.ts`](../src/app/api/v1/corrections/route.ts)) instead of overwriting it silently:

- **Ownership-only, like granting consent.** The caller must be the record's own patient (`actor.patientProfileId === record.patientId`) — this is a direct check, not routed through `canAccess()`, the same pattern already used for `Consent` creation (correcting your own record isn't something a caregiver or provider does on your behalf here).
- **The original value is preserved, not lost.** Before applying the change, the endpoint snapshots only the fields being corrected as `CorrectionRequest.previousValues`, then updates the underlying record and sets `verificationStatus: PATIENT_CORRECTED` — a status distinct from `PATIENT_CONFIRMED` ("the patient accepted an AI draft as-is") because a dispute-and-change is a materially different provenance fact. `CorrectionRequest` rows are append-only, exactly like `AuditEvent`/`DataAccessLog` — nothing in the codebase updates or deletes one.
- **`source` never changes.** A provider-entered medication stays `PROVIDER_ENTERED` even after a patient correction — the record's origin is a historical fact; only whether it currently reflects the patient's confirmed-correct value changes.
- **Applies immediately, by design, not by accident.** There's no provider-review UI in this phase to route a correction through first, so it takes effect on submission — every write is still fully attributed (`requestedByUserId`, `reason`, an `AuditEvent` with action `RECORD_CORRECTION_SUBMITTED`) and permanently reversible-in-spirit because the original value is never discarded. This is a scope decision to revisit once a provider-facing review flow exists, not an oversight.
- **Scoped to `correctedFields`, nothing else.** A real bug surfaced during live verification of this feature: Zod's `.partial()` still applies a field's `.default(...)` when that field is simply absent from the input, so `medicationSchema.partial().parse({ dose: "10mg" })` silently filled in `status: "ACTIVE"` even though the caller never touched `status` — which would have both corrupted an unrelated field on unrelated updates (e.g. silently un-pausing a paused medication) and polluted `previousValues`/`correctedValues` with a field nobody asked to change. Fixed by [`pickProvidedFields()`](../src/lib/validation/partial-update.ts), which filters a parsed partial-update result down to the keys the caller actually sent; this applies to the correction endpoint and was also retrofitted onto the pre-existing direct-`PATCH` handlers for medications/conditions/labs, which had the identical bug.
- Scope is the three most clinically central record types (`MEDICATION`, `CONDITION`, `LAB_RESULT`) — see [`src/lib/validation/corrections.ts`](../src/lib/validation/corrections.ts).
- Verified live against the running app: a direct `PATCH` on a `PROVIDER_VERIFIED` seeded medication returns `409`; submitting a correction through the UI updates the record, sets `verificationStatus: PATIENT_CORRECTED`, writes a `CorrectionRequest` with the pre-correction value intact, writes an `AuditEvent`, and appears in the record's correction history — both via the API directly and through the `/medications`, `/conditions`, and `/health` (labs tab) UI.
